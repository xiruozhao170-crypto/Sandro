...
def main():
    ...
    images = []
    for path in sorted((REPO / "figures/task6_wpsh").glob("*.png")):
    ...
    assert len(images) == 3 and len(checks) == 18
    result = dict(status="passed", raw_model_seasons_checked=checks,
                  ...
                  figures=images, svg="All three SVG figures contain no embedded raster images")
