...
- 降水合成：华东 15–45°N / 100–130°E，JJA 三个月求和成季节值，减全时段季节平均得异常（mm/day），
  正位相平均 − 负位相平均 = 合成差；模式降水统一插值到 **GPCC 0.5° 网格**（旧版 npy 的 "ec" 网格）。
...
- `Task3_Figure3_Rainfall_Composite_Spread.png`：各成员合成差 + 跨成员方差，标注与观测合成型的空间相关：
  **r1 = −0.16、r2 = −0.05、r3 = 0.20、r4 = −0.22、r5 = 0.13**
  （自由耦合模式内部变率与观测不同步，低相关属预期）。
- `Task3_Figure4_Positive_Negative_Rainfall_Spread.png`：正/负位相降水异常的跨成员方差。
- **best3 = r3, r5, r2**（相关从高到低取前三）→ 供 Task 4。
...
- best3 平均合成差与历史型的空间相关：ssp245 = 0.19、ssp585 = 0.03；两情景之间 0.02
